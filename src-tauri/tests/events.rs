...
use claude_copet_lib::events::{mood_for_event, parse, Event, Mood};
...
fn valid_jsonl_parses_to_events_and_advances_offset() {
    ...
    let line1: &[u8] =
        b"{\"type\":\"SessionStart\",\"ts\":\"2024-01-01T00:00:00Z\",\"session\":\"s1\"}\n";
    ...
    assert_eq!(
        ...
        "offset must equal buf.len() after consuming all complete lines"
    );
}
...
fn malformed_lines_skipped_not_fatal_offset_advances() {
    ...
    assert_eq!(
        events.len(),
        2,
        "malformed line must be skipped, yielding 2 valid events"
    );
    assert_eq!(events[0].event_type, "SessionStart");
    assert_eq!(events[1].event_type, "Stop");
    assert_eq!(
        offset, expected_offset,
        "offset must advance past malformed line too"
    );
}
...
fn resume_from_offset_is_idempotent_and_unterminated_line_held() {
    ...
    assert_eq!(
        events1.len(),
        1,
        "only the complete line should be emitted on first parse"
    );
    ...
    assert!(
        offset1 < buf.len(),
        "unterminated line must NOT be consumed"
    );
    ...
    assert_eq!(
        events2.len(),
        0,
        "re-parsing from saved offset must yield no new events"
    );
    assert_eq!(
        offset2, offset1,
        "offset must be unchanged when no new complete line is available"
    );
}
...
fn mood_for_event_mapping_matches_spec() {
    ...
    // SessionStart → Wake
    assert_eq!(
        mood_for_event(&make("SessionStart", None)),
        Some(Mood::Wake)
    );

    // UserPromptSubmit → Listen
    assert_eq!(
        mood_for_event(&make("UserPromptSubmit", None)),
        Some(Mood::Listen)
    );

    // PreToolUse → Work
    assert_eq!(
        mood_for_event(&make("PreToolUse", Some("Bash"))),
        Some(Mood::Work)
    );
    ...
    assert_eq!(
        mood_for_event(&make("Notification", None)),
        Some(Mood::Panic)
    );
